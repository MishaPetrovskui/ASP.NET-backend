...
namespace UsersAPI.Controllers
{
    ...
    public class DepartmentsController : ControllerBase
    {
        ...
        public DepartmentsController(UsersDbContext ctx) { _ctx = ctx; }
        ...
        public async Task<IActionResult> GetAll(int page = 1, int size = 10)
        {
            ...
            var total = await _ctx.Departments.CountAsync();
            var items = await _ctx.Departments.OrderBy(d => d.Id).Skip((page - 1) * size).Take(size).ToListAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Department dep)
        {
            _ctx.Departments.Add(dep);
        ...
        {
            var ex = await _ctx.Departments.FindAsync(id);
